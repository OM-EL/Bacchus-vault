...
const addAuctionSell = async () => {
  if (inputsAreValid()) {
    await GovernorContract.methods.updateAuctionSell(
      tokenIdValue,
      auctionPriceValue,
    )
      .send({ from: account })
  ...
  return (
    <div>
      <h4>Update the auction sell price of a bottle</h4>
      <div>
        <input
          type="number"
        ...
        ></input>
        <button onClick={addAuctionSell}>Update auction selling price</button>
      </div>
    </div>
  )
}

export default UpdateAuctionSell;